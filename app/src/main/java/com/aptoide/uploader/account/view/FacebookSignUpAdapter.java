...
public class FacebookSignUpAdapter implements SignUpAdapter<FacebookLoginResult> {
  ...
  @Override
  public Completable signUp(FacebookLoginResult result, AptoideAccountManager accountManager) {

    if (!isEnabled()) {
      return Completable.error(new IllegalStateException("Facebook sign up is not enabled"));
    }
    ...
    if (result.getState() == FacebookLoginResult.STATE_ERROR) {
      return Completable.defer(() -> Completable.error(
    ...
    if (!result.getResult()
      ...
      return Completable.error(new FacebookSignUpException(FacebookSignUpException.
          MISSING_REQUIRED_PERMISSIONS, "MISSING_REQUIRED_PERMISSIONS"));
    }

    return getFacebookEmail(result.getResult()
        .getAccessToken()).flatMapCompletable(
        email -> accountManager.login(AptoideAccountManager.TYPE_FACEBOOK, email, result.getResult()
            .getAccessToken()
            .getToken()));
  }

  @Override public Completable logout() {
    return Completable.fromAction(loginManager::logOut);
  }
